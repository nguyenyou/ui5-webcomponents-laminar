...
object Table extends WebComponent("ui5-table") {
  ...
  /** Defines if the table is in busy state.
    *
    * In this state the component's opacity is reduced and busy indicator is
  ...
  lazy val busy: HtmlAttr[Boolean] =
    htmlAttr("busy", BooleanAsAttrPresenceCodec)
  ...
  /** Defines the delay in milliseconds, after which the busy indicator will
    ...
    * Default: 1000
    */
  lazy val busyDelay: HtmlAttr[Double] =
    htmlAttr("busy-delay", DoubleAsStringCodec)

  /** Defines whether the table will have growing capability either by pressing
    * a `More` button, or via user scroll. In both cases `load-more` event is
    * fired.
    *
    * Available options:
    *
    * `Button` - Shows a `More` button at the bottom of the table, pressing of
    * which triggers the `load-more` event.
    *
    * `Scroll` - The `load-more` event is triggered when the user scrolls to the
    * bottom of the table;
    *
    * `None` (default) - The growing is off.
    *
    * **Restrictions:** `growing="Scroll"` is not supported for Internet
    * Explorer, and the component will fallback to `growing="Button"`.
    *
    * **Note:** Available since
    * [v2.0.0](https://github.com/SAP/ui5-webcomponents/releases/tag/v2.0.0) of
    * **@ui5/webcomponents-compat**.
    ...
    * Default: "None"
    */
  lazy val growing: HtmlAttr[TableGrowingMode] =
    htmlAttr("growing", StringUnionCodec[TableGrowingMode])
  ...
  /** Defines the subtext that will be displayed under the `growingButtonText`.
    ...
    * Default: undefined
    */
  lazy val growingButtonSubtext: HtmlAttr[String] =
    htmlAttr("growing-button-subtext", StringAsIsCodec)

  /** Defines the text that will be displayed inside the growing button at the
    * bottom of the table, meant for loading more rows upon press.
    *
  ...
  /** Defines if the value of `noDataText` will be diplayed when there is no
    ...
    * Default: false
    */
  lazy val hideNoData: HtmlAttr[Boolean] =
    htmlAttr("hide-no-data", BooleanAsAttrPresenceCodec)
  ...
  /** Defines the mode of the component.
    ...
    * Default: "None"
    */
  lazy val mode: HtmlAttr[TableMode] =
    htmlAttr("mode", StringUnionCodec[TableMode])
  type TableMode = "None" | "SingleSelect" | "MultiSelect"

  /** Defines the text that will be displayed when there is no data and
    * `hideNoData` is not present.
    *
    * Default: undefined
    */
  ...
  /** Determines whether the column headers remain fixed at the top of the page
    * during vertical scrolling as long as the Web Component is in the viewport.
    *
    * **Restrictions:**
    *
    ...
    *   - Scrolling behavior:
    *
    *   - If the Web Component is placed in layout containers that have the
    *     `overflow: hidden` or `overflow: auto` style definition, this can
    *     prevent the sticky elements of the Web Component from becoming fixed
  ...
  /** Fired when the user presses the `More` button or scrolls to the table's
    * end.
    *
  ...
  /** Fired when `TableColumn` is shown as a pop-in instead of hiding it.
    ...
    * |     ❌      |    ✅    |
    */
  lazy val onPopinChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[TablePopinChangeEventDetail]
  ] = new EventProp("popin-change")
  ...
  /** Fired when a row in `Active` mode is clicked or `Enter` key is pressed.
    ...
    */
  lazy val onRowClick
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[TableRowClickEventDetail]] =
    new EventProp("row-click")

  /** Fired when selection is changed by user interaction in `SingleSelect` and
    * `MultiSelect` modes.
    *
    * **Note:** Available since
    * [v2.0.0](https://github.com/SAP/ui5-webcomponents/releases/tag/v2.0.0) of
  ...
  lazy val onSelectionChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[TableSelectionChangeEventDetail]
  ...
  /** Defines the configuration for the columns of the component.
    ...
    * attribute (`slot="columns"`). Since you can't change the DOM order of
