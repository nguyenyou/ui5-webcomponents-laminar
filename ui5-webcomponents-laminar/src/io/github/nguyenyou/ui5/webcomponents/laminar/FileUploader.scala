...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distFileUploaderMod.{
...
object FileUploader extends WebComponent("ui5-file-uploader") {
  ...
  /** Comma-separated list of file types that the component should accept.
    *
    * **Note:** Please make sure you are adding the `.` in front on the file
    * type, e.g. `.png` in case you want to accept png's only.
    *
    * Default: undefined
    */
  ...
  /** If set to "true", the input field of component will not be rendered. Only
    * the default slot that is passed will be rendered.
    *
    * Default: false
    */
  lazy val hideInput: HtmlAttr[Boolean] =
    htmlAttr("hide-input", BooleanAsAttrPresenceCodec)

  /** Defines the maximum file size in megabytes which prevents the upload if at
    * least one file exceeds it.
    *
    ...
    * Default: undefined
    */
  lazy val maxFileSize: HtmlAttr[Double] =
    htmlAttr("max-file-size", DoubleAsStringCodec)

  /** Allows multiple files to be chosen.
    *
    * Default: false
    */
  lazy val multiple: HtmlAttr[Boolean] =
    htmlAttr("multiple", BooleanAsAttrPresenceCodec)
  ...
  /** Event is fired when the value of the file path has been changed.
    *
    * **Note:** Keep in mind that because of the HTML input element of type
    * file, the event is also fired in Chrome browser when the Cancel button of
    * the uploads window is pressed.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
  ...
  lazy val onChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[FileUploaderChangeEventDetail]
  ...
  /** Event is fired when the size of a file is above the `maxFileSize` property
    ...
    * |     ❌      |    ✅    |
    */
  lazy val onFileSizeExceed: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[FileUploaderFileSizeExceedEventDetail]
