...
object Tab extends WebComponent("ui5-tab") {
  ...
  /** Represents the "additionalText" text, which is displayed in the tab. In the cases when in the same time there are
    * tabs with icons and tabs without icons, if a tab has no icon the "additionalText" is displayed larger.
    *
    * Default: undefined
    */
  ...
  /** Defines the icon source URI to be displayed as graphical element within the component. The SAP-icons font provides
    * numerous built-in icons. See all the available icons in the [Icon
  ...
  /** Defines hierarchies with nested sub tabs.
    ...
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot) attribute (`slot="items"`). Since
