...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distCardHeaderMod.{
...
object CardHeader extends WebComponent("ui5-card-header") {
  ...
  /** Defines if the component would be interactive, e.g gets hover effect and
    * `click` event is fired, when pressed.
    *
    * Default: false
    */
  ...
  /** Fired when the component is activated by mouse/tap or by using the Enter
    * or Space key.
    *
    * **Note:** The event would be fired only if the `interactive` property is
    * set to true.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
