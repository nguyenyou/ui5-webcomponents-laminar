...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distMultiInputMod.{
  MultiInput as MultiInputComponent
}
...
object MultiInput extends WebComponent("ui5-multi-input") {
  ...
  /** Sets the maximum number of characters available in the input field.
    *
    * **Note:** This property is not compatible with the ui5-input type
    * InputType.Number. If the ui5-input type is set to Number, the maxlength
    * value is ignored.
    *
    * Default: undefined
    */
  ...
  /** Defines whether the suggestions picker is open. The picker will not open
    * if the `showSuggestions` property is set to `false`, the input is disabled
    * or the input is readonly. The picker will close automatically and `close`
    * event will be fired if the input is not in the viewport.
    *
    * **Note:** Available since
  ...
  /** Defines whether the component should show suggestions, if such are
    * present.
    *
    * Default: false
    */
  lazy val showSuggestions: HtmlAttr[Boolean] =
    htmlAttr("show-suggestions", BooleanAsAttrPresenceCodec)

  /** Determines whether a value help icon will be visualized in the end of the
    * input. Pressing the icon will fire `value-help-trigger` event.
    *
    * Default: false
    */
  ...
  /** Defines the HTML type of the component.
    ...
    * **Notes:**
    *
    *   - The particular effect of this property differs depending on the
    *     browser and the current language settings, especially for type
    *     `Number`.
    *   - The property is mostly intended to be used with touch devices that use
    *     different soft keyboard layouts depending on the given input type.
    *
    * Default: "Text"
    */
  lazy val tpe: HtmlAttr[InputType] =
    htmlAttr("type", StringUnionCodec[InputType])
  ...
  /** Fired when the value of the component changes at each keystroke, and when
    * a suggestion item has been selected.
    *
  ...
  /** Fired when the user navigates to a suggestion item via the ARROW keys, as
    * a preview, before the final selection.
    *
    * **Note:** Available since
    * [v2.0.0](https://github.com/SAP/ui5-webcomponents/releases/tag/v2.0.0) of
    * **@ui5/webcomponents**.
  ...
  lazy val onSelectionChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[InputSelectionChangeEventDetail]
  ...
  lazy val onTokenDelete: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[MultiInputTokenDeleteEventDetail]
  ] = new EventProp("token-delete")

  /** Fired when the value help icon is pressed and F4 or ALT/OPTION +
    * ARROW_UP/ARROW_DOWN keyboard keys are used.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onValueHelpTrigger: EventProp[Ui5CustomEvent[Ref]] = new EventProp(
    "value-help-trigger"
  )
  ...
  /** Defines the component tokens.
    ...
    * by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot)
    * attribute (`slot="tokens"`). Since you can't change the DOM order of slots
    * when declaring them within a prop, it might prove beneficial to manually
    * mount them as part of the component's children, especially when facing
    * problems with the reading order of screen readers.
    *
