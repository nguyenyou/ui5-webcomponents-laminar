...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distListItemStandardMod.ListItemStandard as ListItemStandardComponent
...
object ListItemStandard extends WebComponent("ui5-li") {
  ...
  /** Defines the text alternative of the component. Note: If not provided a default text alternative will be set, if
    * present.
    *
    * Default: undefined
    */
  ...
  /** **Note:** While the slot allows option for setting custom avatar, to match the design guidelines, please use the
    * `Avatar` with it's default size - S.
    *
    * **Note:** If bigger `Avatar` needs to be used, then the size of the `ListItemStandard` should be customized in
    * order to fit.
    *
    * __Note:__ The content of the prop will be rendered into a
    * [&lt;slot&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) by assigning the respective
