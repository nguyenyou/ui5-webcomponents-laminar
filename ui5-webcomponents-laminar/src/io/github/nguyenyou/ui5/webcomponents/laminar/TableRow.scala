...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distTableRowMod.{
...
object TableRow extends WebComponent("ui5-table-row") {
  ...
  /** Defines the position of the row related to the total number of rows within
    * the table when the `TableVirtualizer` feature is used.
    *
    * **Note:** Available since
    ...
    * Default: undefined
    */
  lazy val position: HtmlAttr[Double] =
    htmlAttr("position", DoubleAsStringCodec)

  /** Unique identifier of the row.
    *
    * **Note:** For selection features to work properly, this property is
    * mandatory, and its value must not contain spaces.
    *
    * Default: undefined
    */
  ...
  /** Defines the actions of the component.
    *
    * **Note:** Use `TableRowAction` or `TableRowActionNavigation` for the
    * intended design.
    *
    ...
    * attribute (`slot="actions"`). Since you can't change the DOM order of
