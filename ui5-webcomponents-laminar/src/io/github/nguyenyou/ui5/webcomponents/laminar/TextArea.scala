...
object TextArea extends WebComponent("ui5-textarea") {
  ...
  /** Defines the maximum number of rows that the component can grow.
    ...
    * Default: 0
    */
  lazy val growingMaxRows: HtmlAttr[Double] = htmlAttr("growing-max-rows", DoubleAsStringCodec)
  ...
  /** Defines the value state of the component.
    *
    * **Note:** If `maxlength` property is set, the component turns into "Critical" state once the characters exceeds
    * the limit. In this case, only the "Negative" state is considered and can be applied.
    *
    * Default: "None"
    */
  ...
  /** Defines the value state message that will be displayed as pop up under the component. The value state message slot
    ...
    *
    * **Note:** The `valueStateMessage` would be displayed if the component has `valueState` of type `Information`,
