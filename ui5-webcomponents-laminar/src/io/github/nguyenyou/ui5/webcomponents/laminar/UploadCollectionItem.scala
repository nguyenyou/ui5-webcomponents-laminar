...
import io.github.nguyenyou.ui5.webcomponents.ui5WebcomponentsFiori.distUploadCollectionItemMod.{
  UploadCollectionItem as UploadCollectionItemComponent
}
...
object UploadCollectionItem extends WebComponent("ui5-upload-collection-item") {
  ...
  /** If set to `true` the file name will be clickable and it will fire
    * `file-name-click` event upon click.
    *
    * Default: false
    */
  ...
  /** The upload progress in percentage.
    ...
    * Default: 0
    */
  lazy val progress: HtmlAttr[Double] =
    htmlAttr("progress", DoubleAsStringCodec)
  ...
  /** Upload state.
    ...
    *   - `Uploading` - progress indicator and terminate button are displayed.
    *     When the terminate button is pressed, `terminate` event is fired.
    *   - `Error` - progress indicator and retry button are displayed. When the
    *     retry button is pressed, `retry` event is fired.
    *   - `Complete` - progress indicator is not displayed.
    *
    * Default: "Ready"
    */
  lazy val uploadState: HtmlAttr[UploadState] =
    htmlAttr("upload-state", StringUnionCodec[UploadState])
  ...
  /** Fired when the file name is clicked.
    *
    * **Note:** This event is only available when `fileNameClickable` property
    * is `true`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onFileNameClick: EventProp[Ui5CustomEvent[Ref]] = new EventProp(
    "file-name-click"
  )

  /** Fired when the `fileName` property gets changed.
    *
    * **Note:** An edit button is displayed on each item, when the
    * `UploadCollectionItem` `type` property is set to `Detail`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
  ...
  /** Fired when the retry button is pressed.
    *
    * **Note:** Retry button is displayed when `uploadState` property is set to
    * `Error`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
  ...
  /** Fired when the terminate button is pressed.
    *
    * **Note:** Terminate button is displayed when `uploadState` property is set
    * to `Uploading`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onTerminate: EventProp[Ui5CustomEvent[Ref]] = new EventProp(
    "terminate"
  )
