...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distTreeItemCustomMod.{
  TreeItemCustom as TreeItemCustomComponent
}
...
object TreeItemCustom extends WebComponent("ui5-tree-item-custom") {
  ...
  /** Defines whether the tree list item will show a collapse or expand icon
    * inside its toggle button.
    *
    * Default: false
    */
  ...
  /** Defines whether the tree node has children, even if currently no other
    * tree nodes are slotted inside.
    *
    * **Note:** This property is useful for showing big tree structures where
    * not all nodes are initially loaded due to performance reasons. Set this to
    * `true` for nodes you intend to load lazily, when the user clicks the
    * expand button. It is not necessary to set this property otherwise. If a
    * tree item has children, the expand button will be displayed anyway.
    *
    * Default: false
    */
  lazy val hasChildren: HtmlAttr[Boolean] =
    htmlAttr("has-children", BooleanAsAttrPresenceCodec)
  ...
  lazy val hideSelectionElement: HtmlAttr[Boolean] =
    htmlAttr("hide-selection-element", BooleanAsAttrPresenceCodec)
  ...
  /** Defines whether the selection of a tree node is displayed as partially
    * selected.
    *
    * **Note:** The indeterminate state can be set only programmatically and
    * can’t be achieved by user interaction, meaning that the resulting visual
    * state depends on the values of the `indeterminate` and `selected`
    * properties:
    *
    *   - If a tree node has both `selected` and `indeterminate` set to `true`,
    *     it is displayed as partially selected.
    *   - If a tree node has `selected` set to `true` and `indeterminate` set to
    *     `false`, it is displayed as selected.
    *   - If a tree node has `selected` set to `false`, it is displayed as not
    *     selected regardless of the value of the `indeterminate` property.
    *
    * **Note:** This property takes effect only when the `Tree` is in `Multiple`
    * mode.
    *
    * **Note:** Available since
