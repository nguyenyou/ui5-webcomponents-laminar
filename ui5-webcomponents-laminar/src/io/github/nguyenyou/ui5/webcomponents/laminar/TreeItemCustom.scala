...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distTreeItemCustomMod.TreeItemCustom as TreeItemCustomComponent
...
object TreeItemCustom extends WebComponent("ui5-tree-item-custom") {
  ...
  /** Defines whether the tree list item should display the selection element.
    ...
    * Default: false
    */
  lazy val hideSelectionElement: HtmlAttr[Boolean] = htmlAttr("hide-selection-element", BooleanAsAttrPresenceCodec)
  ...
  /** **Note:** While the slot allows option for setting custom avatar, to match the design guidelines, please use the
    * `Avatar` with size XS.
    *
    * **Note:** If bigger `Avatar` needs to be used, then the size of the `TreeItem` should be customized in order to
    * fit.
    *
    * __Note:__ The content of the prop will be rendered into a
    * [&lt;slot&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) by assigning the respective
