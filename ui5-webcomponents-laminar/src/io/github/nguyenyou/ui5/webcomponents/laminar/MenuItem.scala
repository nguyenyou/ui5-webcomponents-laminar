...
object MenuItem extends WebComponent("ui5-menu-item") {
  ...
  /** Defines the delay in milliseconds, after which the loading indicator will
    ...
    *
    * **Note:** If set to `true` a `BusyIndicator` component will be displayed
    * into the related one to the current `MenuItem` sub-menu popover.
    *
    * **Note:** Available since
  ...
  /** Defines the components that should be displayed at the end of the menu
    ...
    * `Button`,`Link` or `Icon` in order to preserve the intended design. If
    * there are components added to this slot, and there is text set in
    * `additionalText`, it will not be displayed. If there are items added to
