...
import io.github.nguyenyou.ui5.webcomponents.ui5Webcomponents.distCalendarMod.{
...
object Calendar extends WebComponent("ui5-calendar") {
  ...
  /** Defines the type of selection used in the calendar component. Accepted
    * property values are:
    *
    *   - `CalendarSelectionMode.Single` - enables a single date
    *     selection.(default value)
    *   - `CalendarSelectionMode.Range` - enables selection of a date range.
    *   - `CalendarSelectionMode.Multiple` - enables selection of multiple
    *     dates.
    *
    * Default: "Single"
    */
  ...
  /** Fired when the selected dates change.
    *
    * **Note:** If you call `preventDefault()` for this event, the component
    * will not create instances of `CalendarDate` for the newly selected dates.
    * In that case you should do this manually.
    *
  ...
  lazy val onSelectionChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[CalendarSelectionChangeEventDetail]
  ...
  /** Defines the calendar legend of the component.
    ...
    * attribute (`slot="calendarLegend"`). Since you can't change the DOM order
  ...
  /** Defines the special dates, visually emphasized in the calendar.
    ...
    * by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot)
    * attribute (`slot="specialDates"`). Since you can't change the DOM order of
