...
object UploadCollection extends WebComponent("ui5-upload-collection") {
  ...
  /** By default there will be drag and drop overlay shown over the
    * `UploadCollection` when files are dragged. If you don't intend to use drag
    * and drop, set this property.
    *
    * **Note:** It is up to the application developer to add handler for `drop`
    * event and handle it. `UploadCollection` only displays an overlay.
    *
    * Default: false
    */
  lazy val hideDragOverlay: HtmlAttr[Boolean] =
    htmlAttr("hide-drag-overlay", BooleanAsAttrPresenceCodec)

  /** Allows you to set your own text for the 'No data' description.
    *
    * Default: undefined
    */
  lazy val noDataDescription: HtmlAttr[String] =
    htmlAttr("no-data-description", StringAsIsCodec)
  ...
  type UploadCollectionSelectionMode = "None" | "Single" | "SingleStart" |
  ...
  /** Fired when an element is dropped inside the drag and drop overlay.
    *
    * **Note:** The `drop` event is fired only when elements are dropped within
    * the drag and drop overlay and ignored for the other parts of the
    * `UploadCollection`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
  ...
  lazy val onItemDelete: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[UploadCollectionItemDeleteEventDetail]
  ] = new EventProp("item-delete")

  /** Fired when selection is changed by user interaction in `Single` and
    * `Multiple` modes.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
  ...
  lazy val onSelectionChange: EventProp[
    Ui5CustomEvent[Ref] &
      EventDetail[UploadCollectionSelectionChangeEventDetail]
  ...
  /** Defines the `UploadCollection` header.
    *
    * **Note:** If `header` slot is provided, the labelling of the
    * `UploadCollection` is a responsibility of the application developer.
