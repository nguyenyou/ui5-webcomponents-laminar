...
object Table extends WebComponent("ui5-table") {
  ...
  /** Defines the mode of the <code>ui5-table</code> overflow behavior.
    ...
    *
    * <code>Scroll</code> - Columns are shown as regular columns and horizontal
    * scrolling is enabled. <code>Popin</code> - Columns are shown as pop-ins
    * instead of regular columns.
    *
    * Default: "Scroll"
    */
  lazy val overflowMode: HtmlAttr[TableOverflowMode] =
    htmlAttr("overflow-mode", StringUnionCodec[TableOverflowMode])
  type TableOverflowMode = "Scroll" | "Popin"

  /** Defines the maximum number of row actions that is displayed, which
    * determines the width of the row action column.
    *
    * **Note:** It is recommended to use a maximum of 3 row actions, as
    * exceeding this limit may take up too much space on smaller screens.
    *
    ...
    * Default: 0
    */
  lazy val rowActionCount: HtmlAttr[Double] =
    htmlAttr("row-action-count", DoubleAsStringCodec)
  ...
  /** Fired when a movable list item is dropped onto a drop target.
    ...
    * **Notes:**
    *
    * The `move` event is fired only if there was a preceding `move-over` with
    * prevented default action.
    *
    * If the dragging operation is a cross-browser operation or files are moved
    * to a potential drop target, the `source` parameter will be `null`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onMove
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[TableMoveEventDetail]] =
    new EventProp("move")

  /** Fired when a movable item is moved over a potential drop target during a
    * dragging operation.
    *
    * If the new position is valid, prevent the default action of the event
    * using `preventDefault()`.
    *
    * **Note:** If the dragging operation is a cross-browser operation or files
    * are moved to a potential drop target, the `source` parameter will be
    * `null`.
    *
  ...
  /** Fired when a row action is clicked.
    ...
    * |     ❌      |    ✅    |
    */
  lazy val onRowActionClick: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[TableRowActionClickEventDetail]
  ...
  /** Defines the features of the component.
    ...
    * attribute (`slot="features"`). Since you can't change the DOM order of
  ...
  /** Defines the header row of the component.
    ...
    * by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot)
    * attribute (`slot="headerRow"`). Since you can't change the DOM order of
    * slots when declaring them within a prop, it might prove beneficial to
    * manually mount them as part of the component's children, especially when
    * facing problems with the reading order of screen readers.
    *
  ...
  /** Defines the custom visualization if there is no data available.
    ...
    * by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot)
    * attribute (`slot="noData"`). Since you can't change the DOM order of slots
