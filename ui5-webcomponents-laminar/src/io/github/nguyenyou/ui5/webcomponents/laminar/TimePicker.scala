...
object TimePicker extends WebComponent("ui5-time-picker") {
  ...
  /** Fired when the input operation has finished by clicking the "OK" button or when the text in the input field has
    * changed and the focus leaves the input field.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
  ...
  /** Defines the value state message that will be displayed as pop up under the `TimePicker`.
    ...
    *
    * **Note:** The `valueStateMessage` would be displayed, when the `TimePicker` is in `Information`, `Critical` or
