...
object ComboBox extends WebComponent("ui5-combobox") {
  ...
  /** Fired when selection is changed by user interaction
    ...
    * |     ❌      |    ✅    |
    */
  lazy val onSelectionChange: EventProp[Ui5CustomEvent[Ref] & EventDetail[ComboBoxSelectionChangeEventDetail]] =
  ...
  /** Defines the value state message that will be displayed as pop up under the component. The value state message slot
    ...
    *
    * **Note:** The `valueStateMessage` would be displayed, when the `ComboBox` is in `Information`, `Critical` or
