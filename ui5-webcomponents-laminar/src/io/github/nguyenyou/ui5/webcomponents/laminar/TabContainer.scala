...
object TabContainer extends WebComponent("ui5-tabcontainer") {
  ...
  type Ref = TabContainerComponent with dom.HTMLElement
  ...
  /** Sets the background color of the Tab Container's content as `Solid`,
  ...
  /** Sets the background color of the Tab Container's header as `Solid`,
    * `Transparent`, or `Translucent`.
    *
    * **Note:** Available since
    * [v1.10.0](https://github.com/SAP/ui5-webcomponents/releases/tag/v1.10.0)
    ...
    * Default: "Solid"
    */
  lazy val headerBackgroundDesign: HtmlAttr[BackgroundDesign] =
    htmlAttr("header-background-design", StringUnionCodec[BackgroundDesign])

  /** Defines if automatic tab selection is deactivated.
    *
    * **Note:** By default, if none of the child tabs have the `selected`
    * property set, the first tab will be automatically selected. Setting this
    * property to `true` allows preventing this behavior.
    *
    * **Note:** Available since
    ...
    * Default: false
    */
  lazy val noAutoSelection: HtmlAttr[Boolean] =
    htmlAttr("no-auto-selection", BooleanAsAttrPresenceCodec)

  /** Defines the overflow mode of the header (the tab strip). If you have a
    * large number of tabs, only the tabs that can fit on screen will be
    * visible. All other tabs that can 't fit on the screen are available in an
    * overflow tab "More".
    *
    * **Note:** Only one overflow at the end would be displayed by default, but
    * when set to `StartAndEnd`, there will be two overflows on both ends, and
    * tab order will not change on tab selection.
    *
    * **Note:** Available since
    ...
    * Default: "End"
    */
  lazy val overflowMode: HtmlAttr[OverflowMode] =
    htmlAttr("overflow-mode", StringUnionCodec[OverflowMode])
  type OverflowMode = "End" | "StartAndEnd"

  /** Defines the alignment of the content and the `additionalText` of a tab.
    *
    * **Note:** The content and the `additionalText` would be displayed
    * vertically by default, but when set to `Inline`, they would be displayed
    * horizontally.
    *
    * Default: "Standard"
    */
  lazy val tabLayout: HtmlAttr[TabLayout] =
    htmlAttr("tab-layout", StringUnionCodec[TabLayout])
  ...
  /** Fired when element is moved to the tab container.
    ...
    * |     ❌      |    ✅    |
    */
  lazy val onMove: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[TabContainerMoveEventDetail]
  ] = new EventProp("move")
  ...
  /** Fired when a tab is selected.
    ...
    * |     ✅      |    ✅    |
    */
  lazy val onTabSelect: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[TabContainerTabSelectEventDetail]
  ] = new EventProp("tab-select")

  // -- Slots --

  /** Defines the button which will open the overflow menu. If nothing is
    * provided to this slot, the default button will be used.
    *
  ...
  /** Defines the button which will open the start overflow menu if available.
    ...
    * by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot)
    * attribute (`slot="startOverflowButton"`). Since you can't change the DOM
