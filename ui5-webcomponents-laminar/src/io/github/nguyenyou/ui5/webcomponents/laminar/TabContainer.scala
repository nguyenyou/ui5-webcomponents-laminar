...
object TabContainer extends WebComponent("ui5-tabcontainer") {
  ...
  /** Defines if automatic tab selection is deactivated.
    *
    * **Note:** By default, if none of the child tabs have the `selected` property set, the first tab will be
    * automatically selected. Setting this property to `true` allows preventing this behavior.
    *
  ...
  /** Defines the overflow mode of the header (the tab strip). If you have a large number of tabs, only the tabs that
    * can fit on screen will be visible. All other tabs that can 't fit on the screen are available in an overflow tab
    * "More".
    *
    * **Note:** Only one overflow at the end would be displayed by default, but when set to `StartAndEnd`, there will be
    * two overflows on both ends, and tab order will not change on tab selection.
    *
  ...
  /** Defines the alignment of the content and the `additionalText` of a tab.
    *
    * **Note:** The content and the `additionalText` would be displayed vertically by default, but when set to `Inline`,
  ...
  /** Fired when element is being moved over the tab container.
    ...
    * |     ✅      |    ✅    |
    */
  lazy val onMoveOver: EventProp[Ui5CustomEvent[Ref] & EventDetail[TabContainerMoveEventDetail]] = new EventProp(
    "move-over"
  )
  ...
  /** Fired when a tab is selected.
    ...
    * |     ✅      |    ✅    |
    */
  lazy val onTabSelect: EventProp[Ui5CustomEvent[Ref] & EventDetail[TabContainerTabSelectEventDetail]] = new EventProp(
    "tab-select"
  )

  // -- Slots --

  /** Defines the button which will open the overflow menu. If nothing is provided to this slot, the default button will
    * be used.
    *
  ...
  /** Defines the button which will open the start overflow menu if available. If nothing is provided to this slot, the
    * default button will be used.
    *
    * __Note:__ The content of the prop will be rendered into a
    * [&lt;slot&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) by assigning the respective
    * [slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/slot) attribute
    * (`slot="startOverflowButton"`). Since you can't change the DOM order of slots when declaring them within a prop,
