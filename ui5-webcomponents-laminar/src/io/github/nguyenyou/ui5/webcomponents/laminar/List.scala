...
object List extends WebComponent("ui5-list") {
  ...
  /** Defines the IDs of the elements that describe the component.
    ...
    * Default: undefined
    */
  lazy val accessibleDescriptionRef: HtmlAttr[String] =
    htmlAttr("accessible-description-ref", StringAsIsCodec)
  ...
  /** Defines the footer text.
    ...
    * Default: undefined
    */
  lazy val footerText: HtmlAttr[String] =
    htmlAttr("footer-text", StringAsIsCodec)

  /** Defines whether the component will have growing capability either by
    * pressing a `More` button, or via user scroll. In both cases `load-more`
    * event is fired.
    *
    * **Restrictions:** `growing="Scroll"` is not supported for Internet
    * Explorer, on IE the component will fallback to `growing="Button"`.
    *
    * Default: "None"
    */
  lazy val growing: HtmlAttr[ListGrowingMode] =
    htmlAttr("growing", StringUnionCodec[ListGrowingMode])
  ...
  /** Defines the text that will be displayed inside the growing button.
    ...
    *
    * **Note:** This property takes effect if the `growing` property is set to
    * the `Button`.
    *
    * **Note:** Available since
    ...
    * Default: undefined
    */
  lazy val growingButtonText: HtmlAttr[String] =
    htmlAttr("growing-button-text", StringAsIsCodec)
  ...
  /** Determines whether the component is indented.
    ...
    * Default: false
    */
  lazy val indent: HtmlAttr[Boolean] =
    htmlAttr("indent", BooleanAsAttrPresenceCodec)
  ...
  /** Defines the selection mode of the component.
    ...
    * Default: "None"
    */
  lazy val selectionMode: HtmlAttr[ListSelectionMode] =
    htmlAttr("selection-mode", StringUnionCodec[ListSelectionMode])
  type ListSelectionMode = "None" | "Single" | "SingleStart" | "SingleEnd" |
    "SingleAuto" | "Multiple" | "Delete"

  /** Defines the item separator style that is used.
    *
    * Default: "All"
    */
  lazy val separators: HtmlAttr[ListSeparator] =
    htmlAttr("separators", StringUnionCodec[ListSeparator])
  type ListSeparator = "All" | "Inner" | "None"

  // -- Events --

  /** Fired when an item is activated, unless the item's `type` property is set
    * to `Inactive`.
    *
    * **Note**: This event is not triggered by interactions with selection
    * components such as the checkboxes and radio buttons, associated with
    * non-default `selectionMode` values, or if any other **interactive**
    * component (such as a button or input) within the list item is directly
    * clicked.
    *
  ...
  lazy val onItemClick
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[ListItemClickEventDetail]] =
    new EventProp("item-click")

  /** Fired when the `Close` button of any item is clicked
    *
    * **Note:** This event is only applicable to list items that can be closed
    * (such as notification list items), not to be confused with `item-delete`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onItemClose
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[ListItemCloseEventDetail]] =
    new EventProp("item-close")

  /** Fired when the Delete button of any item is pressed.
    *
    * **Note:** A Delete button is displayed on each item, when the component
    * `selectionMode` property is set to `Delete`.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onItemDelete: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[ListItemDeleteEventDetail]
  ] = new EventProp("item-delete")

  /** Fired when the `Toggle` button of any item is clicked.
    *
    * **Note:** This event is only applicable to list items that can be toggled
    * (such as notification group list items).
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
  ...
  lazy val onItemToggle: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[ListItemToggleEventDetail]
  ] = new EventProp("item-toggle")

  /** Fired when the user scrolls to the bottom of the list.
    *
    * **Note:** The event is fired when the `growing='Scroll'` property is
    * enabled.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
  ...
  /** Fired when a movable list item is dropped onto a drop target.
    *
    * **Note:** `move` event is fired only if there was a preceding `move-over`
    * with prevented default action.
    *
    * | cancelable | bubbles |
    * |:----------:|:-------:|
    * |     ❌      |    ✅    |
    */
  lazy val onMove
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[ListMoveEventDetail]] =
    new EventProp("move")

  /** Fired when a movable list item is moved over a potential drop target
    * during a dragging operation.
    *
    * If the new position is valid, prevent the default action of the event
  ...
  lazy val onMoveOver
      : EventProp[Ui5CustomEvent[Ref] & EventDetail[ListMoveEventDetail]] =
    new EventProp("move-over")

  /** Fired when selection is changed by user interaction in `Single`,
    * `SingleStart`, `SingleEnd` and `Multiple` selection modes.
    *
    * **Note:** Call `event.preventDefault()` inside the handler of this event
    * to prevent its default action/s.
    *
  ...
  lazy val onSelectionChange: EventProp[
    Ui5CustomEvent[Ref] & EventDetail[ListSelectionChangeEventDetail]
