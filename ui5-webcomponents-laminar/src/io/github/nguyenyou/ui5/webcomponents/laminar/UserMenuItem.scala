...
object UserMenuItem extends WebComponent("ui5-user-menu-item") {
  ...
  /** Defines the components that should be displayed at the end of the menu item.
    *
    * **Note:** It is highly recommended to slot only components of type `ui5-button`,`ui5-link` or `ui5-icon` in order
    * to preserve the intended design. If there are components added to this slot, and there is text set in
    * `additionalText`, it will not be displayed. If there are items added to `items` slot, nether `additionalText` nor
    * components added to this slot would be displayed.
    *
    * The priority of what will be displayed at the end of the menu item is as follows: sub-menu arrow (if there are
    * items added in `items` slot) -> components added in `endContent` -> text set to `additionalText`.
    *
