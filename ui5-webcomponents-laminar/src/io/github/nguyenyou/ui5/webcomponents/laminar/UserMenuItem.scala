...
import io.github.nguyenyou.ui5.webcomponents.ui5WebcomponentsFiori.distUserMenuItemMod.{
  UserMenuItem as UserMenuItemComponent
}
...
object UserMenuItem extends WebComponent("ui5-user-menu-item") {
  ...
  /** Defines the `additionalText`, displayed in the end of the menu item.
    *
    * **Note:** The additional text will not be displayed if there are items
    * added in `items` slot or there are components added to `endContent` slot.
    *
    * The priority of what will be displayed at the end of the menu item is as
    * follows: sub-menu arrow (if there are items added in `items` slot) ->
    * components added in `endContent` -> text set to `additionalText`.
    *
    * **Note:** Available since
  ...
  /** Defines the delay in milliseconds, after which the loading indicator will
    ...
    *
    * **Note:** If set to `true` a `ui5-busy-indicator` component will be
    * displayed into the related one to the current `ui5-menu-item` sub-menu
    * popover.
    *
    * **Note:** Available since
  ...
  /** Defines the delete button, displayed in "Delete" mode. **Note:** While the
    ...
    * `ui5-button` component. **Note:** When the slot is not present, a built-in
  ...
  /** Defines the components that should be displayed at the end of the menu
    * item.
    *
    * **Note:** It is highly recommended to slot only components of type
    * `ui5-button`,`ui5-link` or `ui5-icon` in order to preserve the intended
    * design. If there are components added to this slot, and there is text set
    * in `additionalText`, it will not be displayed. If there are items added to
    * `items` slot, nether `additionalText` nor components added to this slot
    * would be displayed.
    *
    * The priority of what will be displayed at the end of the menu item is as
    * follows: sub-menu arrow (if there are items added in `items` slot) ->
